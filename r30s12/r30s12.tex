...
(\S\ref{sec:r30s12-trips}). The REB was left off, and with it all three of the
CCDs it reads.
...
(\S\ref{sec:r30s12-solution}), and has been in effect since 2025-06-25. The
cost is one CCD out of the three on Reb1.
...
noise level (\S\ref{sec:r30s12-ramp}). The REB measures each segment's current
on the low side of its current source, so a segment whose output node is held
near 0~V reads $\simeq0$~mA. The discussion of these data points to a short
between the output node of the affected segment (OS, the source of the output
transistor) and FS, which is at or near ground; a short involving OD was also
raised as plausible, given the large OD current seen at turn-on. An ITL sensor
showed the same signature about five years ago and was concluded to have a
...
its frames are read out and written even though they carry no signal. The granularity of the mitigation is set
by the electronics rather than by the fault: OD can only be lowered for a
whole CCD, so a single shorted segment costs a sensor.
