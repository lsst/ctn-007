...
\item[Feb 7--11] MTP data fibres at the camera bulkhead were found
  ...
  best-effort basis (IT-5920, IT-5885).
...
\item[Mar 14--18] The first attempt to power the camera on the TMA
  ...
  TMA fibre, and power-on was achieved on Mar~18 (IT-6016).
...
\item[Mar 28--31] PCS chiller~2 tripped twice when its upstream
  ...
  (OBS-826).  Restarting the chiller after the breaker work took
...
\item[Apr 13--25] The filter carousel developed an intermittent
  ...
  collector was replaced on Apr~24--25 (SUMMIT-10680), with the
...
\item[May 8--11] The camera lost essentially all cryo cooling capacity
  overnight (OBS-931, FRACAS-280).  The cryo plate began deviating
...
\item[May 21] A power glitch stuck the glycol remote control valve
  ...
  of four running cryo circuits (FRACAS-282).  With the CCS shell
...
\item[May 15--16] Two software faults cost observing time: a 0.1\,s
  ...
  minimum (OBS-947), and a broadcast packet storm from three camera
  hosts faulted the camera (OBS-950).
...
\item[Jun 2--3] The shutter jammed part-open in the middle of a flat
  ...
  HCU were power-cycled and the shutter recalibrated (OBS-1013).
...
\item[Jun 3--4] Dynalene flow was lost overnight, forcing an
  ...
  to the second Dynalene chiller (OBS-1014).  During recovery HV was
...
\item[Jun 10--11] A screw dropped into the camera body through the FES
  access during Auto Changer PLC work (SUMMIT-10747) triggered a
...
\item[Jul 4] A power outage briefly interrupted both glycol and
  ...
  anomaly overnight (OBS-1074), prompting discussion of resilience to
  future outages.
...
\item[Jul 13--28] Repeated \texttt{ocs-bridge} failures to release the
  ...
  time on several nights (OBS-1098, OBS-1079, OBS-1119), tentatively
...
\item[Aug 23--27] A new automated HV-bias ramp-up and regulation
  ...
  temporarily lowering its setpoint (LSSTCAM-124, LSSTCAM-125) and
  resolved by Aug~26.
...
\item[Aug 26--27] Large-scale summit OS and Kubernetes patching
  (SUMMIT-11198) forced a reboot cascade across the camera servers.  A
...
\item[Sep 1--5] The camera recovered from the generator failure, whose
  ...
  (FRACAS-299); the summit server-room UPS had separately exhausted its
  roughly 30-minute battery and dropped camera network communications
  (FRACAS-300).  Recurring generator voltage glitches caused repeated
...
\item[Sep 7] Observers reported an unfamiliar creaking sound over the
  shutter microphone during large camera rotations, with correlated
  spikes in camera-hexapod motor currents; the M2 hexapod, camera
  hexapod and rotator bearings were all considered as possible
  sources (OBS-1158).
...
\item[Sep 14--18] A cluster of DAQ and control-system faults occurred:
  ...
  time in a month (LSSTCAM-130), leaving the DAQ data store
...
\item[Sep 23--25] The $r$ and $g$ filters were extracted ahead of the
  October carousel maintenance (SUMMIT-11343), leaving $i$, $z$ and
...
\item[Oct 15--20] Cryo~3 developed a capillary or expansion-valve clog
  ...
  were planned in response (LCA-14625, LCA-17726).
...
\item[Oct 20--21] The camera returned to on-sky operation.  The first
  ...
  (OBS-1013), incorrect guider bias voltages in the new guider
  firmware, which was rolled back, a recurrence of a long-standing
  wavefront-REB power-on issue (LSSTCAM-37), and corrupted FITS
...
\item[Nov 1--3] A subsystem network fault pegged the REB-power process
  ...
  (LSSTCAM-144; recovery procedure in CTN-003).
...
\item[Nov 6] A dynamic guider region-of-interest test crashed the
  ...
  recurred on a second attempt (LSSTCCSRAFTS-821).
...
\item[Nov 20] A generator-to-commercial power switchover caused a
  ...
  shutter PTP resynchronisation (FRACAS-321).
...
\item[Nov 21--24] The focal-plane subsystem repeatedly hung in an
  ...
  restart (LSSTCAM-148); a parallel memory-exhaustion event in the
  message broker triggered an overnight jGroups storm (LSSTCAM-149,
  LSSTCAM-150).  On Nov~24 the DAQ link on one data-collection node was
  ...
  the switch power-cycled (LSSTCAM-151).
...
\item[Dec 15--16] Water droplets and streaks were found on M1M3, and a
  ...
  the dew point exceeded the heat-exchanger return temperature (SUMMIT-11795,
  OBS-1479).
...
\item[Dec 16--17] Two Dynalene chiller outages, the first coinciding
  ...
  restart to clear data corruption (FRACAS-328).
...
\item[Dec 22] A changeover between Dynalene chillers warmed the supply
  ...
  an hour until cooling was restored by switching back (OBS-1490).
...
\item[Dec 27--29] The science REBs were switched again to the 6.4\,ns
  ...
  a firmware or boot-sequence cause (LSSTCAM-153).
